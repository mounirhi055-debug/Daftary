...
function bindEvents() {
  ...
  document.getElementById("txProduct")?.addEventListener("change", onTxProductSelected);
  UI.enhanceSelect("txProduct");
    ...
    c.transactions.slice(0, timelineShowCount).forEach(t => {
      ...
      item.innerHTML = `
        ...
        <div class="tl-body">
          ...
          ${t.note ? `<div class="tl-note">${UI.escapeHtml(t.note)}</div>` : ""}
        </div>
...
function populateTxProductSelect() {
  ...
  if (!select) return;
  select.innerHTML = `<option value="">— بدون اختيار سلعة —</option>` +
    productsCache.map(p => `<option value="${p.id}">${UI.escapeHtml(p.name)} — ${UI.money(p.price)}</option>`).join("");
}

function onTxProductSelected() {
  const select = document.getElementById("txProduct");
  const product = productsCache.find(p => p.id === select.value);
  if (!product) return;
  document.getElementById("txAmount").value = product.price;
  const noteInput = document.getElementById("txNote");
  if (!noteInput.value.trim()) noteInput.value = product.name;
}
...
async function onSaveTransaction(e) {
  ...
  const note = type === "credit" ? document.getElementById("txNote").value.trim() : "";
  ...
  try {
    const actorName = currentRole === "employee" ? (currentEmployeeName || "موظف") : (currentShop.ownerName || "صاحب المحل");
    const productId = document.getElementById("txProduct")?.value || null;
    await DB.Customers.addTransaction(currentShop.id, activeCustomerId, { type, amount, note, addedByName: actorName, addedByRole: currentRole, productId });
