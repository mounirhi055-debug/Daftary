...
// أيقونات SVG بسيطة (بدل الإيموجي) لأزرار الإجراءات — مظهر موحّد واحترافي.
const ICONS = {
  trash: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>`,
  edit: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>`,
  openArrow: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><path d="M7 17 17 7"/><path d="M8 7h9v9"/></svg>`,
  box: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z"/><path d="M3.3 7 12 12l8.7-5"/><path d="M12 22V12"/></svg>`,
  userSm: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="12" height="12"><path d="M20 21a8 8 0 0 0-16 0"/><circle cx="12" cy="7" r="4"/></svg>`,
  chevronDown: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" width="15" height="15"><path d="m6 9 6 6 6-6"/></svg>`,
  chevronUp: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" width="15" height="15"><path d="m18 15-6-6-6 6"/></svg>`
};
let reminderShowCount = PAGE_SIZE;
let limitNearShowCount = PAGE_SIZE;
let salesLogShowCount = PAGE_SIZE;
let productsShowCount = PAGE_SIZE;
let employeesShowCount = PAGE_SIZE;
let activityLogShowCount = PAGE_SIZE;
...
function renderLoadMoreButton(container, totalCount, shownCount, onLoadMore, onShowLess) {
  ...
  if (shownCount < totalCount) {
    ...
    btn.innerHTML = `${ICONS.chevronDown} عرض المزيد <span class="lm-count">${totalCount - shownCount}</span>`;
  ...
  if (shownCount > PAGE_SIZE && onShowLess) {
    ...
    lessBtn.innerHTML = `${ICONS.chevronUp} عرض أقل`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // أقصى مدة يُسمح فيها بالعمل أوفلاين بالاعتماد على بيانات محفوظة غير
  // مؤكَّدة، بدون أي تحقق حي ناجح من السيرفر — بعدها يُطلب اتصال حقيقي
  // للتأكد من صلاحية الحساب، حتى لا يبقى حساب منتهي يعمل إلى الأبد لمجرد
  // أنه لم يتصل بالإنترنت. نفس المبدأ المستخدم بتطبيقات مثل Spotify للأوفلاين.
  const OFFLINE_GRACE_MS = 3 * 24 * 60 * 60 * 1000; // 3 أيام
  const LAST_VERIFIED_KEY = "daftary_last_verified_active";

  function markVerifiedActiveNow() {
    try { localStorage.setItem(LAST_VERIFIED_KEY, String(Date.now())); } catch (e) {}
  }

  function showReverifyScreen() {
    const container = document.querySelector(".main-area") || document.body;
    container.innerHTML = `
      <div style="max-width:420px; margin:15vh auto; text-align:center; padding:24px;">
        <div style="font-size:2.4rem; margin-bottom:10px;">🔒</div>
        <h3 style="margin-bottom:10px;">يلزم الاتصال بالإنترنت</h3>
        <p class="hint">مرّت فترة طويلة بدون التحقق من صلاحية حسابك. يرجى الاتصال بالإنترنت مرة واحدة لإعادة التحقق ومتابعة الاستخدام أوفلاين بعدها.</p>
      </div>`;
    window.addEventListener("online", () => location.reload(), { once: true });
  }

  async function tryInitDashboard(user) {
    if (dashboardInitialized) return; // امنع أي تهيئة مكرّرة (مستمعي أحداث/Firestore) عند إعادة المحاولة

    let session;
    try {
      session = await DB.Session.resolveSession(user.uid);
    } catch (err) {
      // فشل القراءة غالباً بسبب انقطاع الاتصال ولا توجد نسخة محفوظة محلياً
      // لهذا الحساب على هذا الجهاز (أول استخدام أوفلاين) — هذا لا يعني أن
      // الحساب غير مفعّل، فلا يجوز تسجيل خروج المستخدم بسببه، بل ننتظر
      // ونعيد المحاولة بمجرد عودة الاتصال دون تسجيل خروج.
      if (!navigator.onLine || err.code === "unavailable") {
        UI.toast("لا يوجد اتصال بالإنترنت، ولم يسبق تحميل بيانات هذا الحساب على هذا الجهاز. سيُعاد المحاولة تلقائياً عند عودة الاتصال.", "error");
        window.addEventListener("online", () => tryInitDashboard(user), { once: true });
        return;
      }
      UI.toast(UI.friendlyError(err, "تعذّر تحميل بيانات الحساب"), "error");
      return;
    }
    ...
    // انتهت فترة السماح أوفلاين بدون تحقق حي ناجح من السيرفر — نطلب اتصالاً
    // حقيقياً قبل الاستمرار، بدل الاعتماد إلى ما لا نهاية على بيانات محفوظة.
    let lastVerified = null;
    try { lastVerified = Number(localStorage.getItem(LAST_VERIFIED_KEY)) || null; } catch (e) {}
    if (!lastVerified) {
      markVerifiedActiveNow(); // أول تشغيل بعد هذا التحديث — نمنح فترة سماح كاملة بدل قفل الجميع فوراً
    } else if (Date.now() - lastVerified > OFFLINE_GRACE_MS && !navigator.onLine) {
      showReverifyScreen();
      return;
    }
    ...
    unsubProfile = DB.Shops.watchProfile(currentShop.id, (updated, fromCache) => {
      if (!updated) return;
      currentShop = updated;
      renderUserChip();
      renderExpiryAlert();
      if (updated.status !== "active") {
        // لا نسجّل الخروج إلا لو كانت هذه قراءة مؤكَّدة من السيرفر مباشرة.
        // القراءة المحفوظة محلياً (fromCache) قد تكون قديمة (مثلاً من قبل
        // تجديد الاشتراك، أو ببساطة غير مؤكَّدة أثناء انقطاع الاتصال) — وهذا
        // بالضبط سبب مشكلة "يدخل ثم يخرج" أوفلاين: كانت أي نسخة محفوظة غير
        // مؤكَّدة تُخرج المستخدم فوراً حتى لو الحساب فعّال فعلاً.
        if (fromCache) {
          UI.toast("تعذّر التأكد من حالة تفعيل حسابك بسبب ضعف الاتصال. سيُعاد التحقق تلقائياً عند تحسّن الاتصال.", "error");
          return;
        }
        UI.toast("تم إيقاف تفعيل حسابك من قبل الإدارة", "error");
        setTimeout(() => { DB.Session.signOut(); location.href = "index.html#login"; }, 1200);
      } else if (!fromCache) {
        // تحقّق حي مؤكَّد من السيرفر يفيد بأن الحساب فعّال — نجدّد "ساعة"
        // فترة السماح أوفلاين من هذه اللحظة.
        markVerifiedActiveNow();
      }
    });
  ...
  DB.Session.onChange((user) => {
    if (!user) { location.href = "index.html#login"; return; }
    tryInitDashboard(user);
  });
});
...
function applyRolePermissions() {
  ...
  const txTypeWriteOffLabel = document.getElementById("txTypeWriteOffLabel");
  if (txTypeWriteOffLabel) txTypeWriteOffLabel.style.display = isEmployee ? "none" : "";
}
    ...
    box.innerHTML = `
      <div class="alert alert-warn">
        ...
        <div class="alert-body">
          <strong>باقي ${arabicDaysPhrase(daysLeft)} على انتهاء تفعيل حسابك</strong>
          <p>تواصل مع الإدارة لتجديد الاشتراك وتفادي توقف الخدمة.</p>
        </div>
      </div>`;
  } else {
    box.innerHTML = "";
...
function showView(view) {
  ...
  if (view === "products") { productsShowCount = PAGE_SIZE; renderProducts(); }
  if (view === "salesLog") { salesLogShowCount = PAGE_SIZE; renderSalesLog(); }
  if (view === "stats") renderStats();
  if (view === "account") { renderAccount(); if (currentRole === "owner") { employeesShowCount = PAGE_SIZE; renderEmployees(); } }
  if (view === "notifications") { reminderShowCount = PAGE_SIZE; limitNearShowCount = PAGE_SIZE; renderNotifications(); }
  if (view === "activityLog") { activityLogShowCount = PAGE_SIZE; renderActivityLog(); }
}
...
function bindEvents() {
  ...
  document.getElementById("productSearchInput")?.addEventListener("input", () => { productsShowCount = PAGE_SIZE; renderProducts(); });
  ...
  UI.enhanceSelect("filterSelect");
  UI.enhanceSelect("sortSelect");
  UI.enhanceSelect("salesLogFilterSelect");
  ...
  document.getElementById("txProductQty")?.addEventListener("input", renderTxSelectedStockHint);
  ...
  document.getElementById("txFillFullDebtBtn")?.addEventListener("click", () => {
    const c = customersCache.find(x => x.id === activeCustomerId);
    const balance = c ? DB.Customers.balance(c) : 0;
    if (balance <= 0) {
      UI.toast("لا يوجد دين مستحق على هذا الزبون حالياً", "error");
      return;
    }
    document.getElementById("txAmount").value = balance;
  });
...
function renderCustomers() {
  // بطاقات الإحصائيات أعلى الصفحة يجب أن تعكس إجمالي كل الزبائن دائماً،
  // وليس فقط النتائج المفلترة/المبحوث عنها بالأسفل — لذلك تُحسب من
  // customersCache الكاملة قبل أي فلترة.
  const overallTotals = DB.Customers.shopTotals(customersCache);
  const statDueEl = document.getElementById("statDue");
  const statCreditEl = document.getElementById("statCredit");
  const statPaidEl = document.getElementById("statPaid");
  const statCountEl = document.getElementById("statCount");
  if (statDueEl) statDueEl.textContent = UI.money(overallTotals.totalDue);
  if (statCreditEl) statCreditEl.textContent = UI.money(overallTotals.totalCredit);
  if (statPaidEl) statPaidEl.textContent = UI.money(overallTotals.totalPaid);
  if (statCountEl) statCountEl.textContent = overallTotals.customersCount;
      ...
      tr.innerHTML = `
        ...
        <td class="col-actions">
          <div class="row-actions">
            <button class="icon-btn icon-btn-primary js-open-customer" title="فتح السجل">${ICONS.openArrow}</button>
          </div>
        </td>
      `;
      tr.querySelector(".js-open-customer").addEventListener("click", (e) => { e.stopPropagation(); openCustomerDetail(c.id); });
  ...
  if (c.transactions.length === 0) {
    ...
    c.transactions.slice(0, timelineShowCount).forEach(t => {
      ...
      const itemsDetail = hasItems
        ? `<ul class="tl-items">${t.items.map(it => `<li>${UI.escapeHtml(it.name)} × ${it.qty} — ${UI.money(it.total != null ? it.total : it.price * it.qty)}</li>`).join("")}</ul>`
        : "";

      const typeLabel = t.type === "credit" ? "كريدي جديد" : t.type === "write_off" ? "شطب دين" : "تسديد مبلغ";
      const typeIcon = t.type === "credit" ? "🔴" : t.type === "write_off" ? "✂️" : "🟢";
      const amountSign = t.type === "credit" ? "+" : "−";

      const item = document.createElement("div");
      item.className = "timeline-item";
      item.innerHTML = `
        <div class="tl-icon ${t.type}">${typeIcon}</div>
        <div class="tl-body">
          <div class="tl-top">
            <strong>${typeLabel}</strong>
            <span class="tl-amount ${t.type}">${amountSign}${UI.money(t.amount)}</span>
          </div>
        ...
        <button class="icon-btn icon-btn-danger js-del-tx" title="حذف العملية">${ICONS.trash}</button>
      `;
      ...
      item.querySelector(".js-del-tx")?.addEventListener("click", async () => {
        if (await UI.confirmAction("هل تريد حذف هذه العملية؟")) {
          try {
            await DB.Customers.removeTransaction(currentShop.id, c.id, t);
            logActivity("remove_transaction", `حذف عملية ${t.type === "credit" ? "كريدي" : t.type === "write_off" ? "شطب دين" : "تسديد"} بقيمة ${UI.money(t.amount)} للزبون "${c.name}"`);
...
let txAmountManuallyEdited = false; // المستخدم عدّل المبلغ يدوياً بعد آخر حساب تلقائي من السلع
...
function toggleTxNoteField() {
  ...
  const writeOffHint = document.getElementById("txWriteOffHint");
  const fillFullBtn = document.getElementById("txFillFullDebtBtn");
  const submitBtn = document.getElementById("txSubmitBtn");
  if (!field) return;

  if (productField) productField.style.display = type === "credit" ? "" : "none";
  if (writeOffHint) writeOffHint.style.display = type === "write_off" ? "block" : "none";
  if (fillFullBtn) fillFullBtn.style.display = type === "write_off" ? "inline-flex" : "none";
  if (submitBtn) {
    submitBtn.textContent = type === "write_off" ? "✂️ شطب الدين" : type === "payment" ? "✔ تسجيل التسديد" : "✔ تسجيل الكريدي";
    submitBtn.className = type === "write_off" ? "btn btn-danger-filled btn-block" : "btn btn-primary btn-block";
  }
...
function renderTxSearchResults() {
  ...
  const matches = (q ? productsCache.filter(p => p.name.toLowerCase().includes(q)) : productsCache).slice(0, 8);

  if (productsCache.length === 0) {
    results.innerHTML = `<div class="tx-search-empty">لا توجد سلع في قائمتك بعد</div>`;
  ...
  results.innerHTML = matches.map(p => {
    const stockNote = p.stock == null ? "" : p.stock <= 0 ? ` <span class="tsi-stock out">نفدت</span>` : p.stock <= 5 ? ` <span class="tsi-stock low">متبقٍ ${p.stock}</span>` : "";
    return `
    <div class="tx-search-item" data-id="${p.id}">
      <span class="tsi-name">${UI.escapeHtml(p.name)}${stockNote}</span>
...
function onTxSelectProduct(productId) {
  ...
  document.getElementById("txSelectedProductName").textContent = product.name;
  document.getElementById("txSelectedProductPrice").textContent = UI.money(product.price);
...
function renderTxSelectedStockHint() {
  const hint = document.getElementById("txSelectedStockHint");
  if (!hint) return;
  const product = txSelectedProduct;
  if (!product || product.stock == null) { hint.textContent = ""; hint.className = "tsb-stock-hint"; return; }
  const qty = Math.max(1, parseInt(document.getElementById("txProductQty").value, 10) || 1);
  const already = txCart.filter(it => it.productId === product.id).reduce((s, it) => s + it.qty, 0);
  const remaining = product.stock - already;
  if (qty > remaining) {
    hint.textContent = remaining <= 0 ? "⚠️ نفدت الكمية المتوفرة من هذه السلعة" : `⚠️ المتوفر ${remaining} فقط — سيصبح الرصيد سالباً`;
    hint.className = "tsb-stock-hint warn";
  } else {
    hint.textContent = `المتوفر: ${remaining}`;
    hint.className = "tsb-stock-hint";
  }
}
...
function onTxAddItem() {
  ...
  if (existing) {
    existing.qty += qty;
    if (!existing.manualTotal) existing.total = existing.price * existing.qty;
  } else {
    txCart.push({ productId: product.id, name: product.name, price: product.price, qty, total: product.price * qty, manualTotal: false });
  }
...
function txCartTotal() {
  return txCart.reduce((sum, it) => sum + it.total, 0);
}

function updateTxCartSummary() {
  const totalEl = document.getElementById("txCartTotal");
  const amountInput = document.getElementById("txAmount");
  const total = txCartTotal();
  totalEl.textContent = `المجموع: ${UI.money(total)}`;
  if (!txAmountManuallyEdited) amountInput.value = total;
}
...
function renderTxCart() {
  ...
  list.innerHTML = txCart.map((it, idx) => `
    <li data-idx="${idx}">
      <span class="tci-qty">${it.qty}×</span>
      <span class="tci-name">${UI.escapeHtml(it.name)}</span>
      <span class="tci-leader"></span>
      <span class="tci-price-wrap">
        <input type="number" class="tci-price-input" data-idx="${idx}" value="${it.total}" min="0" step="any" inputmode="decimal">
        <span class="tci-price-suffix">د.ج</span>
      </span>
  ...
  list.querySelectorAll(".tci-price-input").forEach(input => {
    input.addEventListener("input", () => {
      const idx = Number(input.dataset.idx);
      const val = parseFloat(input.value);
      txCart[idx].total = isNaN(val) ? 0 : val;
      txCart[idx].manualTotal = true;
      updateTxCartSummary();
    });
...
async function onSaveTransaction(e) {
  e.preventDefault();
  const type = document.querySelector('input[name="txType"]:checked').value;
  if (type === "write_off" && currentRole !== "owner") return;
  const hasCart = type === "credit" && txCart.length > 0;
  const amount = parseFloat(document.getElementById("txAmount").value);
  let note = type !== "payment" ? document.getElementById("txNote").value.trim() : "";
  if (type === "credit" && !note && hasCart) {
    note = txCart.map(it => it.manualTotal ? `${it.name} (${UI.money(it.total)})` : `${it.name} ×${it.qty}`).join("، ");
  }
  ...
  if ((type === "payment" || type === "write_off") && amount > balance) {
    UI.toast(`المبلغ أكبر من الدين المستحق (${UI.money(balance)}). لا يمكن تسجيل هذه العملية`, "error");
    return;
  }
  ...
  if (type === "write_off") {
    const confirmed = await UI.confirmAction(`هل أنت متأكد من شطب ${UI.money(amount)} من دين "${c ? c.name : "الزبون"}"؟ هذا يعني إسقاط المبلغ نهائياً دون استلامه فعلياً.`);
    if (!confirmed) return;
  }
  ...
  try {
    ...
    const items = hasCart ? txCart.map(it => ({ productId: it.productId, name: it.name, price: it.price, qty: it.qty, total: it.total })) : null;
    ...
    if (hasCart) {
      items.forEach(it => {
        ...
        if (p && p.stock != null) DB.Products.adjustStock(currentShop.id, it.productId, it.qty);
      });
    }
    const actionLabel = type === "credit" ? "تسجيل كريدي" : type === "payment" ? "تسجيل تسديد" : "شطب دين";
    logActivity("add_transaction", `${actionLabel} بقيمة ${UI.money(amount)} للزبون "${c ? c.name : ""}"`);
    resetTxCart();
    UI.closeModal("txModal");
    UI.toast(type === "credit" ? "تم تسجيل الكريدي بنجاح" : type === "payment" ? "تم تسجيل التسديد بنجاح" : "تم شطب الدين بنجاح", "success");
  } catch (err) {
...
function renderActivityLog() {
  ...
  wrap.innerHTML = activityLogCache.slice(0, activityLogShowCount).map(entry => `
    <div class="notif-row">
      <div>
  ...
  `).join("");
  renderLoadMoreButton(wrap, activityLogCache.length, activityLogShowCount, () => {
    activityLogShowCount += PAGE_SIZE;
    renderActivityLog();
  }, () => {
    activityLogShowCount = PAGE_SIZE;
    renderActivityLog();
...
function renderEmployees() {
  ...
  wrap.innerHTML = employeesCache.slice(0, employeesShowCount).map(emp => `
    <div class="notif-row" data-emp-id="${emp.id}">
      ...
      <button class="icon-btn icon-btn-danger js-del-emp" title="حذف الموظف">${ICONS.trash}</button>
    </div>
  `).join("");
  ...
  renderLoadMoreButton(wrap, employeesCache.length, employeesShowCount, () => {
    employeesShowCount += PAGE_SIZE;
    renderEmployees();
  }, () => {
    employeesShowCount = PAGE_SIZE;
    renderEmployees();
...
function openProductModal(product = null) {
  ...
  setProductImagePreview(productImageBase64);
  const lockForEmployee = currentRole === "employee" && !!product;
  const priceField = document.getElementById("productPrice");
  priceField.disabled = lockForEmployee;
  priceField.title = lockForEmployee ? "لا تملك صلاحية تعديل السعر" : "";
  const stockField = document.getElementById("productStock");
  stockField.disabled = lockForEmployee;
  stockField.title = lockForEmployee ? "لا تملك صلاحية تعديل الكمية — يحدّدها صاحب المحل" : "";
...
async function onSaveProduct(e) {
  ...
  if (id && currentRole === "employee") {
    ...
    if (original) { price = original.price; stock = original.stock; }
  }
  ...
  try {
    if (id) {
      await DB.Products.update(currentShop.id, { id, name, price, stock, imageBase64: productImageBase64, note });
      logActivity("update_product", `تعديل السلعة "${name}"`);
      UI.toast("تم تحديث السلعة", "success");
    } else {
      const actorName = currentRole === "employee" ? (currentEmployeeName || "موظف") : (currentShop.ownerName || "صاحب المحل");
      await DB.Products.add(currentShop.id, {
        name, price, stock, imageBase64: productImageBase64, note,
...
function renderProducts() {
  ...
  grid.innerHTML = list.slice(0, productsShowCount).map(p => {
    const stockBadge = p.stock == null ? "" :
      p.stock <= 0 ? `<span class="product-stock-badge out">نفدت الكمية</span>` :
      p.stock <= 5 ? `<span class="product-stock-badge low">متبقٍ ${p.stock} فقط</span>` :
      `<span class="product-stock-badge ok">متوفر: ${p.stock}</span>`;
    return `
    <div class="product-card" data-product-id="${p.id}">
      <div class="product-card-img">
        ${p.imageBase64 ? `<img src="${p.imageBase64}" alt="${UI.escapeHtml(p.name)}">` : `<span class="product-card-noimg">${ICONS.box}</span>`}
        ${stockBadge}
      </div>
      <div class="product-card-body">
        ...
        <div class="product-card-meta">${ICONS.userSm} أضافها: ${UI.escapeHtml(p.addedByName || "—")}</div>
      </div>
      <div class="product-card-actions">
        <button class="icon-btn icon-btn-primary js-edit-product" title="تعديل">${ICONS.edit}</button>
        <button class="icon-btn icon-btn-danger js-del-product" title="حذف">${ICONS.trash}</button>
      </div>
    </div>
  `;
  }).join("");
  ...
  gridWrap.querySelector(".load-more-wrap")?.remove();
  renderLoadMoreButton(gridWrap, list.length, productsShowCount, () => {
    productsShowCount += PAGE_SIZE;
    renderProducts();
  }, () => {
    productsShowCount = PAGE_SIZE;
    renderProducts();
...
function renderSalesLog() {
  ...
  if (entries.length === 0) {
    ...
    entries.slice(0, salesLogShowCount).forEach(en => {
      ...
      const isWriteOff = en.type === "write_off";
      const badgeCls = isCredit ? "badge-red" : isWriteOff ? "badge-purple" : "badge-green";
      const badgeLabel = isCredit ? "🔴 كريدي" : isWriteOff ? "✂️ شطب" : "🟢 تسديد";
      const amountCls = isCredit ? "due" : isWriteOff ? "writeoff" : "clear";
      ...
      tr.innerHTML = `
        ...
        <td class="col-sl-type"><span class="tx-type-badge ${badgeCls}">${badgeLabel}</span></td>
        <td><span class="balance ${amountCls}">${amountSign}${UI.money(en.amount)}</span></td>
