...
document.addEventListener("DOMContentLoaded", () => {
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      document.getElementById("shopLoginForm").style.display = target === "shop" ? "block" : "none";
      document.getElementById("adminLoginForm").style.display = target === "admin" ? "block" : "none";
    });
  });
  ...
  if (shopLoginForm) {
    shopLoginForm.addEventListener("submit", async (e) => {
      ...
      const alertBox = document.getElementById("shopLoginAlert");
      const submitBtn = shopLoginForm.querySelector("button[type=submit]");
      alertBox.innerHTML = "";
      submitBtn.disabled = true;
      try {
        const session = await DB.Session.login(email, password);
  ...
  if (adminLoginForm) {
    adminLoginForm.addEventListener("submit", async (e) => {
      ...
      alertBox.innerHTML = "";
      try {
        DB.LoginGuard.assertNotLocked(email);
        await auth.signInWithEmailAndPassword(email, pass);
        DB.LoginGuard.recordSuccess(email);
        const isAdmin = await DB.Session.isAdmin();
        if (!isAdmin) {
          await auth.signOut();
          throw new Error("هذا الحساب غير مصرَّح له بدخول لوحة الإدارة");
        }
        UI.toast("تم الدخول كإدارة، جاري التحويل...", "success");
        setTimeout(() => location.href = "admin.html", 400);
      } catch (err) {
        if (!err.message.includes("محظور")) DB.LoginGuard.recordFailure(email);
        alertBox.innerHTML = `<div class="alert alert-error">⚠️ ${UI.escapeHtml(UI.friendlyError(err, "بيانات الدخول غير صحيحة"))}</div>`;
      }
    });
  }
