...
const DB = (() => {
  ...
  // get() الافتراضية لا تعتمد دائماً على النسخة المحفوظة محلياً تلقائياً في كل
  // حالات انقطاع الاتصال (تختلف حسب إصدار SDK) — هذه الدالة تضمن صراحةً
  // أن أي مستند سبق تخزينه محلياً (من جلسة سابقة وأنت متصل) يبقى قابلاً
  // للقراءة حتى بدون إنترنت لاحقاً، بدل الاعتماد على سلوك ضمني قد لا يعمل دائماً.
  async function getDocCacheFallback(ref) {
    try {
  ...
  function productFromDoc(docSnap) {
    ...
    return {
      ...
      name: d.name,
      price: d.price,
      stock: d.stock != null ? d.stock : null, // null = الكمية غير مُتابَعة (بلا حد)
  ...
  const Shops = {
    ...
    async getProfile(shopId) {
      const docSnap = await getDocCacheFallback(fs.collection("shops").doc(shopId));
  ...
  const Customers = {
    ...
    async addTransaction(shopId, customerId, { type, amount, note, addedByName, addedByRole, productId, items }) {
      ...
      const cleanItems = Array.isArray(items) && items.length
        ? items.map(it => ({
            ...
            qty: Number(it.qty) || 1,
            total: it.total != null ? Number(it.total) : (Number(it.price) || 0) * (Number(it.qty) || 1)
          }))
        : null;
    ...
    balance(customer) {
      // كريدي يزيد الدين، وأي شيء آخر (تسديد أو شطب) ينقصه.
    ...
    shopTotals(customers) {
      let totalDue = 0, totalCredit = 0, totalPaid = 0, totalWrittenOff = 0;
      customers.forEach((c) => {
        c.transactions.forEach((t) => {
          if (t.type === "credit") totalCredit += t.amount;
          else if (t.type === "write_off") totalWrittenOff += t.amount;
          else totalPaid += t.amount;
        });
      });
      totalDue = totalCredit - totalPaid - totalWrittenOff;
      return { totalDue, totalCredit, totalPaid, totalWrittenOff, customersCount: customers.length };
    }
  };
  ...
  const Products = {
    ...
    async add(shopId, { name, price, stock, imageBase64, note, addedByName, addedByRole }) {
      const ref = await this.col(shopId).add({
    ...
    // فرق موجب = بيع (نقص الكمية)، فرق سالب = استرجاع (عند حذف عملية).
    // لا تُطبَّق إلا على السلع التي تُتابَع كميتها (stock رقم وليس null)،
    // وهي عملية ذرّية (increment) تعمل أوفلاين وتتزامن تلقائياً مثل باقي التعديلات.
    async adjustStock(shopId, productId, delta) {
      if (!productId || !delta) return;
      try {
        await this.col(shopId).doc(productId).update({
          stock: FieldValue.increment(-delta)
        });
      } catch (err) {
        // لو السلعة حُذفت لاحقاً أو كميتها غير متابَعة أصلاً، نتجاهل بصمت —
        // هذا تحديث مصاحب للعملية الرئيسية وليس حرجاً بما يستحق فشل العملية كلها.
        console.warn("تعذّر تحديث كمية السلعة", productId, err);
      }
    }
  };
  ...
  const Employees = {
    ...
    async getProfile(uid) {
      const docSnap = await getDocCacheFallback(fs.collection("employees").doc(uid));
