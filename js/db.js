...
const DB = (() => {
  ...
  const Customers = {
    ...
    async addTransaction(shopId, customerId, { type, amount, note, addedByName, addedByRole, productId, items }) {
      // items: optional array of { productId, name, price, qty } representing a multi-item purchase.
      // When present, `amount` is derived as the sum of (price * qty) so a whole cart
      // becomes a single ledger entry instead of one entry per product.
      const cleanItems = Array.isArray(items) && items.length
        ...
        : null;
      const total = cleanItems
        ? cleanItems.reduce((s, it) => s + it.price * it.qty, 0)
        : Number(amount);
    ...
    async removeTransaction(shopId, customerId, txId) {
      await fs.runTransaction(async (t) => {
        const ref = this.col(shopId).doc(customerId);
        const docSnap = await t.get(ref);
        if (!docSnap.exists) return;
        const list = (docSnap.data().transactions || []).filter((tx) => tx.id !== txId);
        t.update(ref, { transactions: list });
      });
    },
