...
const UI = (() => {
  ...
  function refreshBodyLock() {
    ...
    if (anyOpen && !isLocked) {
      // نحفظ موضع التمرير الحالي قبل القفل، لأن position:fixed يخرج
      // الصفحة من التدفّق الطبيعي ويمنع تسرّب السكرول للصفحة خلف المودال
      // على متصفحات الجوال (overflow:hidden وحدها لا تكفي في أغلبها).
      ...
      // نأجّل استعادة موضع التمرير لإطار رسم واحد حتى يكمل المتصفح
      // إعادة التخطيط بعد رفع position:fixed. نستخدم behavior:"instant"
      // صراحة لأن الموقع يفعّل scroll-behavior:smooth عموماً، وهذا التمرير
      // يجب أن يكون فورياً وغير مرئي (استعادة الحالة، لا حركة تمرير حقيقية).
      requestAnimationFrame(() => window.scrollTo({ top: modalLockScrollY, left: 0, behavior: "instant" }));
    }
  }
